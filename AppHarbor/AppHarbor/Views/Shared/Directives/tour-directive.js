var app;
(function (app) {
    var TourDirective = (function () {
        function TourDirective($rootScope, $cookies, $state, $modal) {
            var _this = this;
            ...
            Sideshow.registerWizard({
                ...
                listeners: {
                    afterWizardEnds: function () { return _this.finishTour(); }
                }
            }).storyLine({
        ...
        TourDirective.prototype.finishTour = function () {
            this.$cookies.put(this.$rootScope.userInfo.id + '.tourComplete', 'true');
            this.$state.go(app.RouteConfig.$routes.Home);
        };
        TourDirective.prototype.link = function (scope, elem, attrs) {
            ...
            if (this.$state.current.name == app.RouteConfig.$routes.Tour) {
                ...
                Sideshow.CloseButton.singleInstance.$el.click(function () { return _this.finishTour(); });
            }
            else if (!tourComplete) {
                this.$state.go(app.RouteConfig.$routes.Tour);
            }
        };
        TourDirective.Injection = 'bootstrapTour';
        TourDirective.$inject = ['$rootScope', '$cookies', '$state', app.GameModalService.Injection];
        return TourDirective;
    }());
    app.TourDirective = TourDirective;
    angular
        .module('app')
